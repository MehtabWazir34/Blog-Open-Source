...
const SignNow = async(request, response)=>{
      try {
        const {fullName, userName,  password} = request.body;
        if(!fullName || !userName || !password){
          response.status(420).json({AlertMsg : 'All fields are required'});
          
        }

        let accountExists = await User.findOne({userName});
        if(accountExists){
          response.status(500).json({AlertMsg: 'Account already exist, plz login'});
        }
        ...
        const token =  jwt.sign({id: newAccount._id}, 'mysuperKey123', {
          expiresIn: '7d'
        })
...
const LoginNow = async(request, response)=>{
  try {
    ...
    if(!theUser){
      return response.status(420).json({
        AlertMsg: 'No account exists with this email, plz create'
      })
    }
    let PasswordMatch = await bcrypt.compare(password, theUser.password)
    if(!PasswordMatch){
      return response.status(430).json({
        AlertMsg: 'Wrong password, try again.'
      })
    }

    let token = jwt.sign({id: theUser._id}, 'mysuperKey123', {
      expiresIn: "7d"
    })
    theUser.token = token;
    await theUser.save();
...
const LogoutNow = async(req, res)=>{
  try {
    let theUser = await User.findByIdAndUpdate(req.user._id, {token:''});
    res.status(200).json({
      AlertMsg:"Logged out."
    })
  } catch (error) {
...
export {SignNow, LoginNow, LogoutNow}